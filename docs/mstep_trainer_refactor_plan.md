# Plan: move closed-form updates into post-optimizer transforms
...
uncertainty to the forward return, and it does not run a second
post-optimizer inference pass. The trainer transform reuses pre-optimizer
posterior/predictive outputs and reconstructs deterministic R quantities from
`moment` plus the current post-optimizer observation/readout state. This is an
accepted lightweight one-step-lagged approximation, not a reason to discard
the refactor.

## Goal

Move closed-form update policy out of `XFADS`, `Observation`, `Noise`, and
`Approx` into configurable trainer-owned post-optimizer transforms.
...
\xrightarrow{\text{Optax optimizer transformation}}
M_k^{\mathrm{opt}}
\xrightarrow{\text{post-optimizer transforms using prior outputs}}
M_{k+1}.
$$

The Gaussian R/Q M-steps are intentionally one-step-lagged: their statistics
come from the pre-optimizer model's forward outputs but are applied to the
post-optimizer model. This avoids a second inference/transition-propagation
pass per batch. The Optax transformation is typically gradient-based, but the
post-optimizer extension point does not assume that it is SGD.
...
The trainer supplies these outputs to all selected transforms. Transforms must
not invoke `model(...)` themselves. `GaussianObservationMstep` reconstructs
the readout mean and propagated posterior variance from `moment` using the
current post-optimizer observation/readout state. This accepted one-step-lagged
...
until a concrete future transform requires one.

## 2. Trainer-owned transform interface

Start with explicit transform objects, not a config-name registry:
...
the transform is a root-level trainer policy.
...
- `post_optimizer_transforms=()`: no post-optimizer transforms or
  transform-owned freeze paths;
- each supplied transform receives the same forward outputs and transforms the
  post-optimizer model in sequence;
- R-only and Q-only experiments are explicit;
- current R/Q M-step transforms commute because they read the same immutable
  outputs and modify disjoint model leaves; sequence order remains explicit
...
Transforms may use narrowly scoped distribution/model helpers but must not
call component M-step methods, recursively trigger another M-step, or invoke
`model(...)`.
...
for transform in post_optimizer_transforms:
    model = transform(model, batch, forward, key=transform_key)
```

`has_aux=True` means `grads` is the gradient of scalar `loss` only; `forward`
is carried out of the differentiated call as auxiliary output for the
post-optimizer transforms. `params` is the pre-optimizer filtered parameter
pytree corresponding to `grads`. Transform-owned frozen leaves are not
optimizer-owned, so a transform update is their final update for the batch.

There is no epoch accumulator, delayed epoch M-step, extra full-data pass, or
extra post-optimizer forward pass in normal training.
...
selected transform's initializer:
...
With `post_optimizer_transforms=()`, XFADS retains its neutral default Q
initialization. Add a separate trainer initializer only if a concrete
optimizer-managed Q experiment needs a configurable initialization policy.
...
```python
def apply_post_optimizer_transforms_from_data(
    model, data, *, post_optimizer_transforms, key
):
    forward = model(*data, key=key)
    for transform in post_optimizer_transforms:
        model = transform(model, data, forward, key=key)
    return model
```
...
The root-level trainer transforms own fully qualified root-relative paths:
...
The trainer concatenates transform paths with user `conf.freeze_paths`.
Components do not expose transform-owned freeze paths and do not know parent
member names.
...
4. `post_optimizer_transforms=()` performs no transform-owned
   initialization, freezing, or updates.
5. Unsupported Approx use fails clearly for `MVNNoiseMstep`.
6. No additional post-optimizer forward call occurs when one or more
   transforms run.
7. Fractional Q formula matches an independent reference.
8. Omitting `MVNNoiseMstep` leaves `noise.free` optimizer-managed while R can
   update.
9. Parameter-aware Optax receives pre-optimizer parameters while
   transform-frozen leaves retain M-step values and Q decomposition uses the
   frozen pre-optimizer Q.
...
1. Add the trainer `PostOptimizerTransform` protocol and
   `post_optimizer_transforms=()` argument.
2. Implement `GaussianObservationMstep` and `MVNNoiseMstep` using existing
   R/Q formulas.
3. Wire loss-and-gradient forward outputs into each transform call.
4. Remove XFADS/Noise/Observation M-step orchestration and obsolete APIs.
5. Migrate freeze paths and configuration/docs.
6. Remove superseded tests and add transform/ordering tests.
