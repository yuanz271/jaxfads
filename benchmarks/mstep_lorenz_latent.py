...
def mstep_transition_diag(model, data, approx, *, floor):
    ...
    t, y, u, c = data
    _, moment, _, _transition_stat = model(t, y, u, c, key=jr.key(0))  # smoothed moments
...
def main():
    ...
    def run(name, *, freeze_q, use_mstep):
        ...
        def on_epoch_end(m, info):
            _, Q = approx.unpack(
                approx.canon_to_moment(approx.free_to_canon(m.noise_free))
            )
        ...
        if freeze_q:
            trainer_conf.freeze_paths = ["noise_free"]
        ...
        if use_mstep:
            ...
            trained = eqx.tree_at(
                lambda mm: mm.noise_free, trained, approx.free_from_kw(scale=q_diag_final)
            )
        ...
        _, Q_final = approx.unpack(
            approx.canon_to_moment(approx.free_to_canon(trained.noise_free))
        )
    ...
    def run_alternating_em(name, *, n_rounds, epochs_per_round, prior, prior_dof_frac):
        ...
        for round_idx in range(n_rounds):
            ...
            model = eqx.tree_at(
                lambda m: m.noise_free, model, approx.free_from_kw(scale=q_shrunk)
            )
            _, Q = approx.unpack(
