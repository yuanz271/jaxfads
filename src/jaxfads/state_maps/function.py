"""Function-backed non-trainable state maps."""
...
def _resolve_from_path(fn_path: str) -> Callable[..., Array]:
    if ":" not in fn_path:
        raise ValueError(
            "FunctionStateMap expects dyn_conf.fn_path in 'module:function' format."
        )
    ...
    try:
        ...
        raise ValueError(
            f"FunctionStateMap could not find symbol '{symbol_name}' in module "
            f"'{module_name}'."
        ) from e
    return fn


class FunctionStateMap(StateMap):
    ...
    def __init__(self, conf, key: Array):
        ...
        if fn_path is None:
            raise ValueError(
                "FunctionStateMap requires `dyn_conf.fn_path` "
        ...
        if not _is_plain_function_or_method(fn):
            raise TypeError(
                "FunctionStateMap expects a plain Python function, method, "
