...
def test_alpha_encoder_shape(spec):
    ...
    out = encoder(y)
    chex.assert_shape(out, (int(conf.free_size),))
...
def test_beta_encoder_shape(spec):
    ...
    out = encoder(a)
    chex.assert_shape(out, (5, int(conf.free_size)))
...
def test_beta_encoder_dropout_path(spec):
    ...
    chex.assert_shape(out, (6, int(conf.free_size)))
    chex.assert_tree_all_finite(out)


def test_beta_encoder_supports_param_and_free_size_contract():
    ...
    encoder = BetaEncoder(conf, jrnd.key(4))

    a = jnp.ones((4, int(conf.param_size)))
    out = encoder(a)
