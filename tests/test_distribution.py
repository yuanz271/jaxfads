...
def test_diagmvn(diag, spec):
    ...
    mp = diag.canon_to_mean(m1, cov1)
    nat1 = diag.mean_to_natural(mp)
    mp1 = diag.natural_to_mean(nat1)

    chex.assert_trees_all_close(mp, mp1)

    mp2 = diag.canon_to_mean(m2, cov2)
    kl = diag.kl(mp1, mp2)
...
def test_diagmvn_near_zero_cov_stability(diag):
    ...
    mp_normal = diag.canon_to_mean(mean, normal_cov)
    natural_normal = diag.mean_to_natural(mp_normal)
    mp_normal_rt = diag.natural_to_mean(natural_normal)
    chex.assert_trees_all_close(mp_normal, mp_normal_rt)
...
def test_diagmvn_kl_matches_closed_form(diag):
    ...
    mp1b = diag.canon_to_mean(m1b, v1b)
    mp2b = diag.canon_to_mean(m2b, v2b)

    kl_actual_b = diag.kl(mp1b, mp2b)
...
def test_fullrank_constrain_natural():
    ...
    mp = full.natural_to_mean(natural)
    chex.assert_tree_all_finite(mp)
...
def test_fullrank_unconstrain_natural_roundtrip():
    ...
    mp = full.natural_to_mean(natural)
    mp_rt = full.natural_to_mean(natural_rt)
    chex.assert_trees_all_close(mp, mp_rt, atol=1e-4)


def test_fullrank_natural_mean_roundtrip():
    """natural ↔ mean roundtrip via structured format."""
    d = 3
    ...
    natural_rt = full.mean_to_natural(mp)
...
def test_diagmvn_unconstrain_mean_roundtrip():
    """constrain_mean(unconstrain_mean(m)) ≈ m for rank=0."""
    state_dim = 4
    diag4 = MVN(dim=state_dim, rank=0)
    mean = jrnd.normal(jrnd.key(10), (state_dim,))
    cov = jnp.abs(jrnd.normal(jrnd.key(11), (state_dim,))) + 0.1
    mp = diag4.canon_to_mean(mean, cov)

    unconstrained = diag4.unconstrain_mean(mp)
    recovered = diag4.constrain_mean(unconstrained)
    chex.assert_trees_all_close(recovered, mp, atol=1e-5)


def test_fullrank_unconstrain_mean_roundtrip():
    """constrain_mean(unconstrain_mean(m)) ≈ m for rank=D."""
    state_dim = 3
    full = MVN(dim=state_dim, rank=state_dim)
    mean = jrnd.normal(jrnd.key(20), (state_dim,))
    cov_diag = jnp.full(state_dim, 2.0)
    cov_factor = jnp.zeros((state_dim, state_dim))
    mp = jnp.concatenate((mean, cov_diag, cov_factor.flatten()))

    unconstrained = full.unconstrain_mean(mp)
    recovered = full.constrain_mean(unconstrained)
    chex.assert_trees_all_close(recovered, mp, atol=1e-4)


def test_init_noise_produces_expected_mean(diag):
    """init_noise → constrain_mean gives correct noise distribution."""
    state_dim = 2
    cov_init = 2.0

    unconstrained = diag.init_noise(cov_init, state_dim)
    mp = diag.constrain_mean(unconstrained)
    mean, var = diag.mean_to_canon(mp)
...
def test_param_size():
    ...
    assert diag3.mean_size(3) == 6


def test_mean_size_full():
    full3 = MVN(dim=3, rank=3)
    assert full3.mean_size(3) == 15
...
def test_lowrank_param_and_mean_sizes():
    d = 4
    lr1 = MVN(dim=d, rank=1)
    lr2 = MVN(dim=d, rank=2)
    assert lr1.param_size(d) == d + d * d
    assert lr1.mean_size(d) == d * (2 + 1)
    assert lr2.mean_size(d) == d * (2 + 2)


def test_lowrank_natural_mean_roundtrip():
    """mean_to_natural → natural_to_mean preserves structure for low-rank MVN."""
    d = 4
    lr1 = MVN(dim=d, rank=1)
    key = jrnd.key(42)
    ...
    mp = jnp.concatenate((loc, cov_diag, cov_factor.flatten()))

    natural = lr1.mean_to_natural(mp)
    chex.assert_tree_all_finite(natural)
    chex.assert_shape(natural, (lr1.param_size(d),))

    mp_rt = lr1.natural_to_mean(natural)
    chex.assert_tree_all_finite(mp_rt)
    chex.assert_shape(mp_rt, (lr1.mean_size(d),))

    _, cov_orig = lr1.mean_to_canon(mp)
    _, cov_rt = lr1.mean_to_canon(mp_rt)
...
def test_lowrank_sample():
    ...
    mp = jnp.concatenate((jnp.zeros(d), jnp.ones(d), cov_factor.flatten()))

    samples = lr2.sample_by_mean(jrnd.key(1), mp, 50)
...
def test_lowrank_constrain_mean_roundtrip():
    """constrain(unconstrain(mp)) ≈ mp for low-rank MVN."""
    d = 4
    lr2 = MVN(dim=d, rank=2)
    ...
    unc = lr2.unconstrain_mean(mp)
    recovered = lr2.constrain_mean(unc)
    chex.assert_trees_all_close(recovered, mp, atol=1e-5)


def test_lowrank_init_noise():
    """init_noise produces isotropic noise mean for low-rank MVN."""
    d = 3
    lr1 = MVN(dim=d, rank=1)
    scale = 2.5
    unc = lr1.init_noise(scale, d)
    mp = lr1.constrain_mean(unc)

    loc, cov_diag, cov_factor = lr1._split_mean(mp)
